...
def locate_checkpoint_files(folder: str | Path) -> list[Path]:
    """Return all ``*_checkpoint.txt`` files found recursively under *folder*."""
    folder = Path(folder)
    return natsorted(f for f in folder.rglob("*_checkpoint.txt") if f.stat().st_size > 0)
